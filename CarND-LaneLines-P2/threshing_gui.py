...
class App:
    ...
    def update(self):
        ...
        if self.play_state:
            ...
            self.add_images([BGR_binary, HSV_binary, HLS_binary, BGR_canny, HSV_canny, HLS_canny, combo_canny])
            #self.frame = cv2.resize(self.frame, (self.width, self.height))
            '''
    ...
    def L2_grad(self):
        if PARAM['L2gradient'] is False:
            PARAM['L2gradient'] = True
        else:
            PARAM['L2gradient'] = False
        
        print(PARAM['L2gradient'])
    ...
    def add_slider_widgets(self):
        ...
        self.add_slider('cannyThresh1', 6, 0, -5000, 5000)
        self.add_slider('cannyThresh2', 6, 1, -5000, 5000)
        self.add_slider('apertureSize', 6, 2, 3, 7)
        self.l2_button = tk.Button(self.window, command=self.L2_grad(), text='L2gradient')
        self.l2_button.grid(row=6, column=3)
    ...
    def add_image_panels(self, photo):
        ...
        self.imagePanel_7 = tk.Label(self.window, common_configs)
        self.imagePanel_7.grid(row=0, column=4, rowspan=4, columnspan=2)
        self.image_panels.append(self.imagePanel_7)
    ...
    def update_sliders(self):
        ...
        for key in label_list.keys():
            for i in range(len(label_list[key])):
                ...
                if key not in oddify_params:
                    PARAM[key][i] = int(self.sliders[row][col].get())
                else: 
                    PARAM[key][i] = oddify(int(self.sliders[row][col].get()))
...
#load_saved_values('carla_params')
